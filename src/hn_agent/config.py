...
class HNAgentSettings(BaseSettings):
    ...
    openai_model: str = Field(
        default="gpt-5-nano", description="OpenAI model to use for summarization"
    )
    openai_max_retries: int = Field(
        default=3, ge=1, le=10, description="Maximum retry attempts for OpenAI API"
    )
    ...
    openai_max_tokens: int = Field(
        default=1000, ge=100, le=4000, description="Maximum tokens for summary"
    )

    # Slack Configuration
    slack_timeout: int = Field(
        default=30, ge=5, le=120, description="Slack webhook timeout in seconds"
    )
    slack_username: str = Field(
        default="HN Bot", description="Bot username for Slack messages"
    )
    slack_icon_emoji: str = Field(
        default=":newspaper:", description="Bot icon emoji for Slack"
    )
    ...
    enable_all_default_topics: bool = Field(
        default=False, description="Enable all predefined default topics"
    )
    ...
    max_stories: int = Field(
        default=50, ge=10, le=200, description="Maximum stories to fetch from HN"
    )
    summary_max_stories: int = Field(
        default=15, ge=5, le=50, description="Maximum stories to include in summary"
    )
    filter_min_score: int = Field(
        default=10, ge=1, description="Minimum story score to consider"
    )
    max_age_hours: float = Field(
        default=24.0, ge=1.0, le=168.0, description="Maximum age of stories in hours"
    )
    ...
    hn_request_delay: float = Field(
        default=0.1, ge=0.0, le=5.0, description="Delay between HN API requests"
    )
    hn_max_concurrent: int = Field(
        default=10, ge=1, le=50, description="Maximum concurrent HN API requests"
    )
    hn_timeout: float = Field(
        default=30.0, ge=5.0, le=120.0, description="HN API request timeout"
    )
    ...
    dry_run: bool = Field(
        default=False, description="Run without sending notifications"
    )
    ...
    def get_effective_topics(self) -> list[str]:
        ...
        topics_list = [
            topic.strip() for topic in self.topics.split(",") if topic.strip()
        ]
        ...
        if self.topic_categories:
            categories_list = [
                cat.strip() for cat in self.topic_categories.split(",") if cat.strip()
            ]
    ...
    def get_log_config(self) -> dict:
        ...
        return {"level": self.log_level, "json": self.json_logs, "debug": self.debug}
    ...
    def validate_configuration(self) -> None:
        ...
        if self.summary_max_stories > self.max_stories:
            raise ValueError("summary_max_stories cannot be greater than max_stories")
