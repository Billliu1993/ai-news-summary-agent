...
class HackerNewsClient:
    ...
    async def get_story(self, story_id: int) -> Optional[Story]:
        ...
        try:
            ...
            if story.type not in (StoryType.STORY, StoryType.ASK, StoryType.SHOW):
                logger.debug("Item is not a story type", 
                           item_id=story_id, item_type=story.type)
                return None
            
            logger.debug("Story fetched successfully", story_id=story_id, title=story.title)
            return story
    ...
    async def get_stories_batch(self, story_ids: List[int]) -> List[Story]:
        ...
        for i, story in enumerate(stories_data):
            if isinstance(story, Exception):
                logger.warning("Story fetch failed", 
                             story_id=story_ids[i], error=str(story))
                error_count += 1
            elif story is not None:
                valid_stories.append(story)
        
        logger.info("Story batch completed", 
    ...
    async def get_recent_stories(
        ...
        logger.info("Recent stories filtered", 
    ...
    async def get_comment(self, comment_id: int) -> Optional[Comment]:
        ...
        try:
            ...
            if comment.type != StoryType.COMMENT:
                logger.debug("Item is not a comment", 
                           item_id=comment_id, item_type=comment.type)
                return None
    ...
    async def get_story_comments(
        ...
        logger.info("Fetching story comments", 
                   story_id=story.id, comment_count=len(comment_ids))
        ...
        logger.info("Story comments fetched", 
                   story_id=story.id, 
